...
| 4. Metadata | validated | `scripts/02-Metadata.ps1`, `make test` | Uses inventory timestamps, optional ExifTool JSON, optional ffprobe media probe. |
...
| 8. Classification | validated | `scripts/06-ClassifyThemes.ps1`, `make test` | Heuristic + LLM with structured JSON parsing; vibe scoring. |
...
| 11. Cleanup | created | `scripts/10-Cleanup.ps1` | Disabled by default; requires safety.allowDelete + -AllowDelete. |
| Python: Entity extraction | created | `scripts/python/10_extract_entities.py` | NER via GLiNER2; requires pip install. |
| Python: Semantic search | created | `scripts/python/11_semantic_search.py` | Vector search via ChromaDB; requires pip install. |
